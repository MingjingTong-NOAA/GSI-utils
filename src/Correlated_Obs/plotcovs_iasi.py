...
def main():
    ...
    for eadj in erradj: 
        ...
        for sen in sensors:
            """ read channel number """
            fileName=f'{datadir}/{eadj}/Error/{fchnum[j]}'
            print (fileName)
            with open(fileName, mode='rb') as f:
                chnum.append(np.fromfile(f,dtype='>i4'))
        
            """ read wave number """
            fileName=f'{datadir}/{eadj}/Error/{fwaven[j]}'
            print (fileName)
            with open(fileName, mode='rb') as f:
                waven.append(np.fromfile(f,dtype='>f4'))
            
            """ read satinfo err """
            fileName=f'{datadir}/{eadj}/Error/{finfoerr[j]}'
            print (fileName)
            with open(fileName, mode='rb') as f:
            ...
            """ read err """
            fileName=f'{datadir}/{eadj}/Error/{ferr[j]}'
            print (fileName)
            with open(fileName, mode='rb') as f: 
                err = np.fromfile(f,dtype='>f4')
                derr.append(err)
            ...
            """ read corr """
            fileName=f'{datadir}/{eadj}/Error/{fcorr[j]}'
            print (fileName)
            ...
            labels.append(f'{eadj}_{sensors[j]}_{stype[j]}')
