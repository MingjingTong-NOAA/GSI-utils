...
contains
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    use vars_calc_analysis, only: mype, do_aero, jedi, cliptracers
    implicit none
    ! variables local to this subroutine
    integer :: i, j, iincvar
    logical :: use_increment,readinc, cliptracer
    real :: clip
    ...
    do i=1,nnciov
      ...
      if (use_increment) then
        if (iovars_netcdf(i) == 'pressfc') then
          ...
          call add_increment(iovars_netcdf(i), incvars_netcdf(iincvar), cliptracer, clip, readinc)
          if (mype==0) then
            if ((.not.jedi) .or. (jedi.and.readinc)) then
               print *, 'Adding Increment to ', iovars_netcdf(i), incvars_netcdf(iincvar), ' cliptracer ', cliptracer
            else
  ...
  subroutine add_increment(fcstvar, incvar, cliptracer, clip, readinc)
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
