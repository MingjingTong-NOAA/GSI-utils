...
integer, parameter :: lunin=11,lunin_pc=14,lunout=12,lunout_pc=13
...
if ( .not. lexist ) then
    ...
    open(lunin_pc, file='abias_pc', form='formatted')
    ...
    abias: do
        ...
        if ( isis(1:4) == 'atms' .or. isis(1:3) == 'mhs' .or. isis(1:5) == 'amsua' ) then
        pred  = 0.0
        tlapm = 0.0
        tsum  = 0.0
        count_tlap = 0
        ...
        read(lunin_pc,102,iostat=istat) &
